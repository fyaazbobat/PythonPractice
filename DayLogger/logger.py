...
def search_command():
    list.delete(0,END)
    for row in backend.search(date_text.get(),sleep_text.get(),exercise_text.get(),study_text.get(),code_text.get(),read_text.get()):
        list.insert(END,row)

def add_command():
    backend.insert(date_text.get(),sleep_text.get(),exercise_text.get(),study_text.get(),code_text.get(),read_text.get())
    list.delete(0,END)
    list.insert(END,(date_text.get(),sleep_text.get(),exercise_text.get(),study_text.get(),code_text.get(),read_text.get()))
...
win.wm_title("My Daily CheckList (CRUD)")

l1 = Label(win, text='Date')
l1.grid(row=0,column=0)

l2 = Label(win, text='Sleep')
...
l5= Label(win, text='Code')
l5.grid(row=2, column=0)

l6 = Label(win, text='Read')
...
e2 = Entry(win,textvariable=sleep_text)
...
code_text = StringVar()
e5 = Entry(win,textvariable=code_text)
e5.grid(row=2,column=1)

read_text = StringVar()
e6 = Entry(win,textvariable=read_text)
e6.grid(row=2,column=3)

list = Listbox(win,height=8,width=35)
list.grid(row=3,column=0,rowspan=9,columnspan=2,pady=5)
